...
public class Pinky extends Ghost {
    ...
    public void Dijkstra(int desti_x, int desti_y, int pac_facing, int[][] tab, int blinky_x, int blinky_y, String mode) {
        if ("frightened".equals(mode)) {
            ...
            // On prédit la position de pacman dans 4 tour :
            switch (pac_facing) {
                case 0 -> {
                    for (int i = 0; i < 4; i++) {
                        if (desti_x + 1 < this.p.tab[0].length && this.p.tab[desti_y][desti_x + 1] != 1) {
                            desti_x++;
                        } else {
                            break;
                        }
                ...
                case 90 -> {
                    for (int i = 0; i < 4; i++) {
                        if (desti_y - 1 > 0 && this.p.tab[desti_y - 1][desti_x] != 1) {
                            desti_y--;
                        } else {
                            break;
                        }
                ...
                case 180 -> {
                    for (int i = 0; i < 4; i++) {
                        if (desti_x - 1 > 0 && this.p.tab[desti_y][desti_x - 1] != 1) {
                            desti_x--;
                        } else {
                            break;
                        }
                ...
                case 270 -> {
                    for (int i = 0; i < 4; i++) {
                        if (desti_y + 1 < this.p.tab.length && this.p.tab[desti_y + 1][desti_x] != 1) {
                            desti_y++;
                        } else {
                            break;
                        }
